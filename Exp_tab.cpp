...
#define YYLSP_NEEDED
#define YYDEBUG             0
//  §p¥è¨âì p áè¨p¥­­yî ¨­ä®p¬ æ¨î ®¡ ®è¨¡ª¥
#if YYDEBUG != 0
#   define YYERROR_VERBOSE
#endif

// ‘«y¦¥¡­ë¥ ¬ ªp®áë ¤«ï yáâ ­®¢«¥­¨ï á®®â¢¥âáâ¢¨© ¬¥¦¤y ¨¬¥­ ¬¨
...
do                                       \
  if (yychar == YYEMPTY && yylen == 1)   \
    ...
    { yyerror ("Ñèíòàêñè÷åñêàÿ îøèáêà: íåëüçÿ âûïîëíèòü back up"); YYERROR; }\
while( 0 );
  ...
  if (yyssp >= yyss + yystacksize - 1)
    {
      ...
      yyoverflow("Ïåðåïîëíåíèå ñòåêà ïàðñåðà", //"parser stack overflow"!!!!!!
      ...
      if (yystacksize >= YYMAXDEPTH)
        {
          yyerror("Ïåðåïîëíåíèå ñòåêà ïàðñåðà"); //"parser stack overflow"!!!!!!
          return 2;
        }
      yystacksize *= 2;
      if (yystacksize > YYMAXDEPTH)
...
#if YYDEBUG != 0
      if (yydebug)
        fprintf(stderr, "Äëèíà ñòåêà óâåëè÷åíà äî %d\n", yystacksize);
      ...
      if (yyn > YYFLAG && yyn < YYLAST)
        {
          ...
          if (msg != 0)
            {
              strcpy(msg, "Ñèíòàêñè÷åñêàÿ îøèáêà");//"parse error"!!!!!!!
              ...
              if (count < 5)
                {
                  ...
                  for (x = (yyn < 0 ? -yyn : 0);
                    ...
                    if (yycheck[x + yyn] == x)
                      {
                        strcat(msg, count == 0 ? ", òðåáóåòñÿ `" : " or `");//expecting!!!!!!!!
            ...
            yyerror ("Îùèáêà ïàðñåðà: èñ÷åðïàíà âèðòóàëüíàÿ ïàìÿòü");
			//"parse error; also virtual memory exceeded"!!!!!!!!!
        }
      else
#endif /* YYERROR_VERBOSE */
        yyerror("Îøèáêà ïàðñåðà");//"parse error"!!!!!!!!
    }
